describe('Chapter 6: ', function() {
  ...
  it('Should compile spinner directive with isolate scope', inject(function($rootScope, $compile) {
    ...
    $rootScope.rating = 3;
    element = angular.element('<adia-spinner ng-model="rating" data-step="2"></adia-spinner>');
    element = $compile(element)($rootScope);
    $rootScope.$digest();
  ...
  it('Should not increment/decrement the value when spinner limit is reached', inject(function($rootScope, $compile) {
    element = angular.element('<adia-spinner ng-model="rating" data-min="1" ng-init="rating = 2" data-max="3"></adia-spinner>');
    element = $compile(element)($rootScope);
    $rootScope.$digest();
  ...
  it('Should work with type: list', inject(function($rootScope, $compile) {
    ...
    element = angular.element('<adia-spinner ng-model="weekday" data-type="list" ng-init="weekday = 2" data-list="availability" on-change="whichDay"></adia-spinner>');
    element = $compile(element)($rootScope);
